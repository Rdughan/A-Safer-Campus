-- Corrected fix for the handle_new_user trigger function
...
-- Create the corrected function (without email field)
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO public.users (id, student_id, username, phone, role, created_at, updated_at)
    VALUES (
        NEW.id,
...
-- Now fix the existing user that's missing (without email field)
...
SELECT id, username, role FROM public.users WHERE id = '477e43bc-9e2f-49a7-b895-c8b9683584f2';
