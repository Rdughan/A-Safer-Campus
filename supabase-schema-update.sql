-- Safe Campus App Database Schema Update
-- This version can be run safely on existing databases
...
DROP TABLE IF EXISTS public.safety_tips CASCADE;
DROP TABLE IF EXISTS public.notifications CASCADE;
DROP TABLE IF EXISTS public.incident_access_logs CASCADE;
DROP TABLE IF EXISTS public.incidents CASCADE;
DROP TABLE IF EXISTS public.incident_institution_mapping CASCADE;
...
DROP POLICY IF EXISTS "Authenticated users can view incident mappings" ON public.incident_institution_mapping;
...
DROP POLICY IF EXISTS "Users can create access logs" ON public.incident_access_logs;
...
DROP TRIGGER IF EXISTS update_safety_tips_updated_at ON public.safety_tips;
...
BEGIN
    ...
    AND u.role IN ('security', 'fire_service', 'medical_service', 'school_management')
    LIMIT 1;
...
DROP TRIGGER IF EXISTS auto_assign_incident_trigger ON public.incidents;
...
-- Success message
SELECT 'Database schema successfully updated! All tables, policies, and sample data have been created.' as status; 